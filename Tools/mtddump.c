...
"Ex: mtddump /dev/mtd6 -f /tmp/mtd6raw\n"
"=>  dump mtd6 raw data and save to file as \"mtdraw6\" in tmp folder\n"
"It is recommend that save the file to external storage drive because the system mem is limited.\n",
...
static bool			pretty_print = true/*flase*/;	// print nice
static bool			noecc = true;		// don't error correct
static bool			omitoob = false/*true*/;		// omit oob data
		...
		unsigned long long prefix)
{
	...
	printf("[David] Enter pretty_dump_to_buffer to dump data...\n");
...
int main(int argc, char * const argv[])
{
	...
	oobbuf = xmalloc(sizeof(oobbuf) * (mtd.min_io_size+mtd.oob_size));
	...
	for (ofs = start_addr; ofs < end_addr; ofs += bs) {
		...
		if (badblock) {
			...
			//printf("[David] nanddump.c: Read page data and exit on failure...\n");
		...
		#if 0
		if (pretty_print) {
			printf("[David] pretty_print (-p) is set and then execute pretty_dump_to_buffer\n");
			...
			printf("[David] not pretty_print and write data to readbuf by command  forcebinary\n");
		...
		if (badblock) {
			...
			//printf("[David] nanddump: mtd_desc= %x ,mtd= %x ,fd= %x ,ofs= %x , rawsize= %x , rawbuf=%x\n", mtd_desc,mtd,fd,ofs,mtd.min_io_size+mtd.oob_size,oobbuf);			
			if (mtd_read_oob(mtd_desc, &mtd, fd, ofs, /*mtd.min_io_size+i*/mtd.oob_size, oobbuf)) {
		...
		*/
			/*Write out page raw data */
			write(ofd, oobbuf, mtd.min_io_size+mtd.oob_size);
		}
	}
